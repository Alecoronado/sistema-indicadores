...
const GanttChart = () => {
  ...
  const datosGantt = useMemo(() => {
    ...
    if (!fechaMin || !fechaMax) {
      fechaMin = new Date(2025, 0, 1); // 1 de enero de 2025
      fechaMax = new Date(2025, 11, 31); // 31 de diciembre de 2025
    }

    // Asegurar que fechaMax sea después de fechaMin y que ambas sean posteriores a 2025
    if (fechaMax <= fechaMin || fechaMin.getFullYear() < 2025) {
      fechaMin = new Date(2025, 0, 1);
      fechaMax = new Date(2025, 11, 31);
    }

    // Generar meses del rango - con límite de seguridad
    const meses = [];
    const fechaActual = new Date(Math.max(fechaMin.getTime(), new Date(2025, 0, 1).getTime()));
    ...
    hitosArray.forEach((hito, hitoIndex) => {
      ...
      let fechaInicioHito = new Date(2025, 0, 1);
      let fechaFinHito = new Date(2025, 11, 31);
      
      if (hito.fechaInicioHito) {
        const tempInicio = new Date(hito.fechaInicioHito);
        if (!isNaN(tempInicio.getTime()) && tempInicio.getFullYear() >= 2025) {
      ...
      if (hito.fechaFinalizacionHito) {
        ...
        if (!isNaN(tempFin.getTime()) && tempFin.getFullYear() >= 2025) {
      ...
      if (fechaFinHito <= fechaInicioHito) {
        fechaFinHito = new Date(fechaInicioHito.getTime() + (30 * 24 * 60 * 60 * 1000)); // Agregar 30 días
      }
